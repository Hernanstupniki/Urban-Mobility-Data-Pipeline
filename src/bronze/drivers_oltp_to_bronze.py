...
# Config
JOB_NAME = "drivers_oltp_to_bronze"
...
def main():
    ...
    try:
        ...
        # 2) Read incremental drivers from OLTP (watermark = updated_at)
        drivers_df = (
            spark.read.format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", "mobility.drivers")
            .option("user", DB_USER)
        ...
        row_count = drivers_df.count()
        max_ts = drivers_df.select(spark_max("updated_at")).first()[0]

        print(f"Wrote {row_count} drivers to Bronze (Delta)")
