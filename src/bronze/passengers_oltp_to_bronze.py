...
# Config
JOB_NAME = "passengers_oltp_to_bronze"
...
def main():
    ...
    try:
        ...
        # 2) Read incremental passengers from OLTP (watermark = updated_at)
        passengers_df = (
            spark.read.format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", "mobility.passengers")
            .option("user", DB_USER)
        ...
        if passengers_df.rdd.isEmpty():
            print("No new passengers to load")
            spark.stop()
            return
        ...
        row_count = passengers_df.count()
        max_ts = passengers_df.select(spark_max("updated_at")).first()[0]

        print(f"Wrote {row_count} passengers to Bronze (Delta)")
