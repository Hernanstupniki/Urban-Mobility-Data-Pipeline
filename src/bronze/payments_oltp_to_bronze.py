...
# Config
JOB_NAME = "payments_oltp_to_bronze"
...
def main():
    ...
    try:
        ...
        # 2) Read incremental payments from OLTP (watermark = updated_at)
        # OJO: la tabla está en schema mobility (por cómo venís con trips)
        payments_df = (
            spark.read.format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", "mobility.payments")
            .option("user", DB_USER)
        ...
        max_ts = payments_df.select(spark_max("updated_at")).first()[0]

        print(f"Wrote {row_count} payments to Bronze (Delta)")
