...
# Config
JOB_NAME = "vehicles_oltp_to_bronze"
...
def main():
    ...
    try:
        ...
        # 2) Read incremental vehicles from OLTP (watermark = updated_at)
        vehicles_df = (
            spark.read.format("jdbc")
            .option("url", JDBC_URL)
            .option("dbtable", "mobility.vehicles")
            .option("user", DB_USER)
        ...
        if vehicles_df.rdd.isEmpty():
            print("No new vehicles to load")
            spark.stop()
            return
        ...
        row_count = vehicles_df.count()
        max_ts = vehicles_df.select(spark_max("updated_at")).first()[0]

        print(f"Wrote {row_count} vehicles to Bronze (Delta)")
