...
# Watermark column for zones (no updated_at in OLTP)
...
def main():
    ...
    try:
        # 1) Watermark (created_at) desde Delta control table
        last_loaded_ts = read_last_loaded_ts(spark)
        print(f"[{JOB_NAME}] last_loaded_ts({WATERMARK_COL}): {last_loaded_ts}")

        # 2) Read incremental zones from OLTP (watermark = created_at)
        zones_df = (
            ...
            .load()
            .filter(col(WATERMARK_COL) > lit(last_loaded_ts))
        )

        if zones_df.rdd.isEmpty():
            print("No new zones to load")
            spark.stop()
            return
        ...
        max_ts = zones_df.select(spark_max(WATERMARK_COL)).first()[0]

        print(f"Wrote {row_count} zones to Bronze (Delta)")
        print(f"[{JOB_NAME}] new watermark({WATERMARK_COL}): {max_ts}")
