...
# Config
JOB_NAME = "dim_passenger_hist_build_gold_conformed"

ENV = os.getenv("ENV", "dev")

SILVER_BASE_PATH = f"data/{ENV}/silver/passengers"
GOLD_BASE_PATH = f"data/{ENV}/gold/_conformed/hist/dim_passenger_hist"

# GDPR configs (opcional, para mantener redacción consistente si llega evento delete)
GDPR_ANON_NAME = os.getenv("GDPR_ANON_NAME", "ANONYMIZED")
GDPR_NULLIFY_CITY = os.getenv("GDPR_NULLIFY_CITY", "1") == "1"
...
def read_target_watermark(spark) -> datetime:
    """
    Watermark = max(raw_loaded_at) in target dim_passenger_hist (SCD2)
    If target doesn't exist or empty -> 1970-01-01
    """
...
def ensure_scd2_fields(df):
    """
    Garantiza columnas SCD2 mínimas:
...
def apply_gdpr_backfill_to_gold_hist(target: DeltaTable, gdpr_events_df):
    """
    Redacta PII en TODA la historia en Gold hist para ids que vienen con is_deleted=true en este batch.
    (Mismo espíritu que tu Silver).
    """
    ...
    # deleted_at: usa el del evento si viene, si no lo que ya tenga, si no now
    ...
    print(f"[{JOB_NAME}] GDPR backfill applied to Gold hist.")
...
def main():
    ...
    try:
        ...
        if not target_exists:
            ...
            print(f"[{JOB_NAME}] dim_passenger_hist created at: {GOLD_BASE_PATH}")
            spark.stop()
            return
        ...
        # 4) Latest per passenger_id inside incremental batch
        ...
        # 7) SCD2 step 2: insert new version (MANUAL estilo Silver 7.2)
        # (Incluye attrs + flags + metadata + SCD2 + audit)
        insert_vals_full = {
        ...
        # 7.3) GDPR backfill (opcional, pero recomendado para consistencia con Silver)
        ...
        print(f"[{JOB_NAME}] dim_passenger_hist SCD2 MERGE completed at: {GOLD_BASE_PATH}")
        spark.stop()
