...
# Config
JOB_NAME = "dim_vehicle_scd3_build_gold_conformed"
...
def read_target_watermark(spark) -> datetime:
    """
    Watermark = max(raw_loaded_at) in target dim_vehicle (SCD3)
...
def main():
    ...
    try:
        ...
        if not target_exists:
            seed_df = build_seed_scd3(silver_all, "vehicle_id")
            ...
            print(f"[{JOB_NAME}] dim_vehicle_scd3 created at: {GOLD_BASE_PATH}")
            spark.stop()
            return
        ...
        dim_df = add_prev_null_columns(dim_df, "vehicle_id")

        target = DeltaTable.forPath(spark, GOLD_BASE_PATH)

        all_cols = dim_df.columns
        current_cols = [c for c in all_cols if c != "vehicle_id" and not c.startswith("prev_")]
