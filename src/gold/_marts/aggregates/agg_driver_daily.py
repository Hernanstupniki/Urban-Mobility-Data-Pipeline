...
Incremental, recompute affected (date_key, driver_key) from fact_trips (late arrivals safe)
...
def read_target_watermark(spark) -> datetime:
    """
    Watermark = max(max_fact_raw_loaded_at) in target agg_driver_daily
...
def main():
    ...
    # 2) Find affected (date_key, driver_key) from NEW facts
    ...
    if "request_date_key" not in fact_inc.columns:
        raise ValueError("request_date_key not found in fact_trips")
    if "driver_key" not in fact_inc.columns:
        raise ValueError("driver_key not found in fact_trips (expected from your fact_trips build)")

    affected = (
        fact_inc
        .select(
            col("request_date_key").cast("int").alias("date_key"),
            col("driver_key").cast("long").alias("driver_key")
        )
        .dropDuplicates(["date_key", "driver_key"])
    )

    affected_count = affected.count()
    print(f"[{JOB_NAME}] affected groups (date_key, driver_key): {affected_count}")

    if affected_count == 0:
        print("No affected groups to process")
        spark.stop()
        return
    ...
    fact_slice = (
        ...
        .join(
            affected,
            (fact_all["request_date_key"].cast("int") == affected["date_key"]) &
            (fact_all["driver_key"].cast("long") == affected["driver_key"]),
            "inner"
        )
        .drop(affected["date_key"])
        .drop(affected["driver_key"])
    )
    ...
    out_count = agg.count()
    print(f"[{JOB_NAME}] output rows (affected groups): {out_count}")
    ...
    # 5) MERGE (upsert by date_key + driver_key)
    target = DeltaTable.forPath(spark, GOLD_BASE_PATH)

    cols = agg.columns
    update_set = {c: f"s.{c}" for c in cols if c not in ["date_key", "driver_key"]}
    insert_vals = {c: f"s.{c}" for c in cols}

    (
        target.alias("t")
        .merge(agg.alias("s"), "t.date_key = s.date_key AND t.driver_key = s.driver_key")
        .whenMatchedUpdate(
